...
function consultationUrl(roomId, suffix = "", appointment = state.appointment) {
  const base = apiUrl(`/api/consultations/${encodeURIComponent(roomId)}${suffix}`);
...
function connectDoctorLiveEvents() {
  ...
  const events = new EventSource(apiUrl("/api/events"), { withCredentials: Boolean(API_BASE) });
...
async function sendConsultMessage(text) {
  ...
  const response = await fetch(consultationUrl(state.appointment.roomId, "/messages"), {
    method: "POST",
    credentials: apiCredentials(),
...
async function postConsultSignal(type, payload = null) {
  if (!state.appointment?.roomId) return;
  await fetch(consultationUrl(state.appointment.roomId, "/signals"), {
    method: "POST",
    credentials: apiCredentials(),
    headers: consultationHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({
