...
function updateBackendLabel() {
  const label = document.querySelector("#adminBackendLabel");
  if (label) label.textContent = backendOriginLabel();
}
...
async function checkSession() {
  try {
    ...
    showLogin(`Backend not connected: ${backendOriginLabel()}`);
  }
}
...
async function loadAdminData(showMessage = false) {
  try {
    ...
    if (!state.doctors.length) {
      listEl.innerHTML = `<article class="admin-card"><div><h3>Backend not connected</h3><p>Admin is trying to connect to <strong>${backendOriginLabel()}</strong>. Check Render deploy and environment variables.</p></div></article>`;
      auditEl.innerHTML = `<div class="audit-item"><strong>Waiting for backend</strong><span>Open the Render backend URL and confirm /api/admin/session responds.</span></div>`;
    }
...
async function loginAdmin(event) {
  ...
  try {
    ...
    if (!response.ok || !payload.authenticated) {
      const baseMessage = payload.message || "Login failed.";
      const hint = response.status === 401
        ? ` Check ADMIN_USERNAME and ADMIN_PASSWORD in Render Environment for ${backendOriginLabel()}.`
        : "";
    ...
    if (loginMessage) loginMessage.textContent = error.message || `Login failed. Backend: ${backendOriginLabel()}`;
  } finally {
...
function getFilteredDoctors() {
  ...
  return state.doctors.filter((doctor) => {
    const haystack = `${doctor.name} ${doctor.specialty} ${doctor.license}`.toLowerCase();
  ...
  listEl.innerHTML = doctors.map((doctor) => {
    ...
    <article class="admin-card status-${status}">
      ...
      <div>
        ...
        <p>Review documents and approve only after license, identity and practice verification checks are complete.</p>
