...
const ALLOWED_ORIGINS = new Set((process.env.CORS_ORIGINS || "https://dishahealthq.in,https://www.dishahealthq.in,http://127.0.0.1:5175,http://localhost:5175")
  .split(",")
...
const seedDoctors = [
  {
    ...
    online: true,
    docs: ["Medical license", "ID proof", "Practice verification"],
  ...
  },
  {
    ...
    docs: ["ID proof pending", "Practice verification"],
...
const nearbyProviders = [
  { id: "h-1", type: "hospital", name: "Partner Care Hospital", lat: 26.1448, lng: 91.7362, distanceKm: 2.4, open: true, tags: ["Bed Support Preview", "ICU Desk", "Emergency Alert"] },
  { id: "h-2", type: "hospital", name: "Disha HealthQ Multi Speciality", lat: 26.1433, lng: 91.7898, distanceKm: 4.1, open: true, tags: ["ICU Preview", "Trauma Desk", "Bed Alerts"] },
  { id: "l-1", type: "lab", name: "Partner Diagnostics Lab", lat: 26.1294, lng: 91.8013, distanceKm: 1.8, open: true, tags: ["Blood Test", "Home Sample", "Reports"] },
  { id: "l-2", type: "lab", name: "Partner Lab Care", lat: 26.1534, lng: 91.7852, distanceKm: 3.2, open: true, tags: ["Lab Booking", "CBC", "Thyroid"] },
  { id: "p-1", type: "pharmacy", name: "Partner Pharmacy", lat: 26.1385, lng: 91.8069, distanceKm: 1.2, open: true, tags: ["Delivery", "Prescription", "24/7"] },
  { id: "p-2", type: "pharmacy", name: "Pharmacy Partner", lat: 26.1844, lng: 91.7409, distanceKm: 4.8, open: false, tags: ["Delivery", "Nearest Store"] },
  { id: "b-1", type: "blood", name: "Emergency Blood Donor Network", lat: 26.1445, lng: 91.7362, distanceKm: 2.9, open: true, tags: ["A+", "O-", "Urgent"] }
];
...
function migrateLegacyJsonData() {
  ...
  removeStoredDoctorLocations();
}

function removeStoredDoctorLocations() {
  if (!db) return;
  const rows = db.prepare("SELECT id, data FROM doctors").all();
  const update = db.prepare("UPDATE doctors SET data = ? WHERE id = ?");
  withDbTransaction(() => {
    rows.forEach((row) => {
      const doctor = parseDbPayload(row, {});
      if (!Object.prototype.hasOwnProperty.call(doctor, "city")) return;
      delete doctor.city;
      update.run(JSON.stringify(doctor), row.id);
    });
  });
}
...
function applyCorsHeaders(req, res) {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, X-CSRF-Token, X-Consult-Token");
...
function cookieHeader(name, value, req, options = {}) {
  ...
  const crossSiteCookie = Boolean(req.headers.origin && ALLOWED_ORIGINS.has(req.headers.origin));
  return [
    `${name}=${encodeURIComponent(value)}`,
    "Path=/",
    "HttpOnly",
    crossSiteCookie ? "SameSite=None" : "SameSite=Strict",
    (secure || crossSiteCookie) ? "Secure" : "",
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/location") {
    ...
    return sendJson(res, 200, {
      ...
      areaLabel: hasValidCoords ? "Nearby care around your current location" : "Location not selected",
